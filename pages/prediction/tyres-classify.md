...
for this question, the knn algorithm is applied for the classifier. 
To determine "best type" for certain weather condition and track, I use the measurement of LapTime to complete a lap to determine this.

-- Import all csv-files into separate tables

First let's extract the weather condition from data, so we will have the table of weather static for each race event
```sql weathers_data
SELECT 
    wd.EventDate,
    wd.EventName,
...
Then get the average lap time based on each compound use in the race
```sql laps_data
select
    CAST(filename[7:10] || '-' || filename[11:12] || '-' || filename[13:14] as date) as EventDate,
    case 
...
From that get the best compound for each race based on the average minimum lap time
...
Combine with the data time with the weather and track data we have
...
Now we take 70% data for train
```sql train_data
select * from ${data_table} where index <=47
```

And 30% data for test
```sql test_data
select * from ${data_table} where index >47
```

Before we can implement the algorithm, we must first apply `min-max-scaling` (this is also called normalizing) to ensure that the values in each column are between 0.0 and 1.0. 

The formula for min-max scaling is `normalized_x = (x - min_x) / (max_x - min_x)`.
...
-- Normalize the training data and store in view
...
-- Join each row from the normalized training view to the **first** row of the normalized test view. You can use a cross join for this. This has already been done for you.
-- Calculate the euclidean distance between the row from the test view and each row from the training view.
...
now we try with second row from the test table
